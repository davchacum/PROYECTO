...
const checkProducts = async (value) => {
	try {
		const productsValid = value.every(product => {
			return Object.prototype.hasOwnProperty.call(product, 'productId') && product.productId > 0 &&
                   Object.prototype.hasOwnProperty.call(product.orderProduct, 'quantity') && product.orderProduct.quantity > 0
		})
		if (!productsValid) {
